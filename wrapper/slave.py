...
def worker(job_queue, res_queue, disas_lst):
    ...
    while True:
        try:
            ...
            bincmp_process.wait()
            res_queue.put((bin_name1, bin_name2, distance))
...
def multithread_bincmp(thread_num, job_queue, disas_lst):
    ...
    for i in range(thread_num):
        thread = threading.Thread(target=worker, args=(job_queue, res_queue, disas_lst))
        thread.start()
